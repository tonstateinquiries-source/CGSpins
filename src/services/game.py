...
def process_influencer_commission(influencer_id: int, referred_user_id: int, package_name: str, payment_type: str = "ton", transaction_id: str = None) -> None:
    ...
    try:
        ...
        if package_key in config.PACKAGES:
            # Use TON price for commission calculation (consistent regardless of payment method)
            package_price = config.PACKAGES[package_key]['price_ton']
            commission_amount = package_price * commission_rate
